...
internally, and the callback is only a notification). See the [Quick start](/docs/quick-start)
for the `defaultData` opt-in and the [API reference](/docs/api-reference) for the exclusivity
rule.
...
all selection, or a clear. It fires from the store actions layer, not a React effect, so it has
no render dependency. It adds no extra subscription in row or cell components (the perf suite's
zero-render probes cover this, see below).
...
"Persist my users' column layout" is a top data-grid request. Without these props, the only path
...
Every event above is outbound-only, from the grid to the consumer. Presence is the odd one out.
...
exactly this path. See [Multiplayer presence](/docs/examples/addons/presence) for the full write-up,
