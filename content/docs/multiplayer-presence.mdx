...
`setPresenceHighlights` takes an array of either form, mixed freely. The `id` must be unique per
entry — a user with a multi-range selection sends one entry per range, each with its own id (there
is no multi-rect field, only one range/cell per array entry). See
...
`GridSelection.current.range`) still works side by side with `rowId` entries in the same array —
useful for a multi-cell selection rect, which the rowId form doesn't express:
...
- The rowId-native form (`RowIdPresenceHighlight`) resolves to a single cell, not a multi-cell
  range — use the `range` form for a multi-cell highlight. Note the trade-off in both directions:
  a `range` entry is in the sender's view space, so a receiver with a different local sort/filter
  paints it on the wrong cells; the rowId-native form is divergence-proof but single-cell only.
...
so a receiver with a different local sort or filter paints them on the wrong cells. The
rowId-native single-cell form is the divergence-proof one, but it only expresses one cell. When
you need divergence-proof sends, broadcast the rowId-native form and map view row to rowId on the
send side with the same core hooks the receive side uses above (`useDataGridRowId` /
`useDataGridRowIds`), called INSIDE the provider. See
