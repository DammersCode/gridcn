...
A plugin is a plain function, not a component and not a hook by name, though it can call hooks
...
seam. They reuse the core's window-clamp and pin-zone-splitting math through the `ctx` argument.
A plugin never re-implements geometry the core already solved.
...
or useMemo) or every render re-syncs the provider props into the store
```
...
[Multiplayer presence](/docs/addons/presence#coordinates-rowid-native-or-view-space).
...
column under a filter, or one flagged by a validation pass elsewhere in your app.
...
wire onto the handle square it renders. See [Fill handle](/docs/addons/fill) and
