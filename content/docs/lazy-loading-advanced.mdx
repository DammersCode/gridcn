...
scrolled back into view later. Two cases apply: a failed fetch's range, and a range you unload
yourself with `lazy.evict(range)`. Use `lazy.getLoadedRanges()` to find the candidates for your
policy. When the evicted range comes back into view, the refetch runs through `fetchRows`. It
serves from cache instead of the network, as long as it is within `staleTime`.
